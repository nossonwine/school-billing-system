...
export default async function StudentProfilePage({ params }: { params: { id: string } }) {
  ...
  const totalCharges = student.incidents.reduce((sum, inc) => sum + inc.feeCalculated, 0);
  const totalPaid = student.incidents.reduce((sum, inc) => sum + inc.amountPaid, 0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            ...
            <form action={saveStudentExemptions} className="flex flex-col gap-3">
              ...
              <textarea 
                ...
                className="w-full border p-2 rounded outline-none focus:ring-2 focus:ring-blue-500 resize-none text-sm"
              />
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            ...
            <form action={addManualCharge} className="flex flex-col gap-3">
              ...
              <input name="date" type="date" required className="w-full border p-2 rounded text-sm" /></div>
              
              <div><label className="text-xs font-bold text-gray-600">Class</label>
              <select name="className" className="w-full border p-2 rounded text-sm bg-white" dir="rtl">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <input name="fee" type="number" step="0.01" required className="w-full border p-2 rounded text-sm" /></div>
              </div>

              <div><label className="text-xs font-bold text-gray-600">Notes (Optional)</label>
              <input name="notes" type="text" placeholder="e.g. Spoke to parents" className="w-full border p-2 rounded text-sm" /></div>
        ...
        <div className="md:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex justify-between items-center mb-4 border-b pb-2">
            ...
            <button onClick={() => {/* Placeholder for next step */}} className="bg-green-600 text-white text-sm font-bold py-1.5 px-4 rounded hover:bg-green-700">
              🖨️ Export to PDF
            </button>
          </div>
              ...
              student.incidents.map((inc) => (
                <div key={inc.id} className={`p-4 border rounded-lg flex flex-col md:flex-row justify-between gap-4 ${inc.isExcused ? 'bg-gray-100 opacity-70' : 'bg-white'}`}>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="font-bold text-lg" dir="rtl">{inc.className}</p>
                    ...
                    <form action={updateChargePayment} className="flex flex-col gap-2 text-sm">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <label className="flex items-center gap-1 text-xs font-bold text-red-600 cursor-pointer">
                          <input type="checkbox" name="isExcused" defaultChecked={inc.isExcused} onChange={(e) => e.target.form?.requestSubmit()} className="w-4 h-4 cursor-pointer" />
                          Excused
                        </label>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input name="amountPaid" type="number" step="0.01" defaultValue={inc.amountPaid} className="w-full border p-1 rounded text-center outline-none focus:ring-2 focus:ring-green-500" />
